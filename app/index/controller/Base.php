...
class Base extends BaseController
{
    ...
    public function getLeftProduct(){
      $archivesData=Db::name('archives')->where('cate_id',2)->limit(4)->order('listorder asc')->order('id desc')->select();  //ishot=1代表推荐度
      return $archivesData;
    }
...
//    }
    ...
    public function getPositionByCatId($cateId){
        $positionData=array();
          while($cateId){
                $cates=Db::name('category')->field('id,cate_name,index_template')->where('id='.$cateId)->find();                   
                $positionData[]=array(
                    'id'=>$cates['id'],
